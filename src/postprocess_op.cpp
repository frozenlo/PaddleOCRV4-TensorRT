...
void PostProcessor::GetContourArea(const std::vector<std::vector<float>> &box, float unclip_ratio, float &distance) {
    int pts_num = 4;
    float area = 0.0f;
    float dist = 0.0f;
...
cv::RotatedRect PostProcessor::UnClip(std::vector<std::vector<float>> box, const float &unclip_ratio) {
    float distance = 1.0;
...
std::vector<std::vector<std::vector<int>>> PostProcessor::BoxesFromBitmap(const cv::Mat &pred, const cv::Mat &bitmap,
...
std::vector<std::vector<std::vector<int>>> PostProcessor::FilterTagDetRes(std::vector<std::vector<std::vector<int>>>& boxes, float ratio_h,
